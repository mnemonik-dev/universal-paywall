...
describe('createReporter', () => {
  ...
  it('skips when the payer is unresolved (no charge)', async () => {
    ...
    expect(await reporter.report({ payerKey: USER, creatorKey: TRACK, amount: 100n })).toEqual({
  ...
  it('rejects non-positive amounts', async () => {
    ...
    expect(await reporter.report({ payerKey: USER, creatorKey: TRACK, amount: 0n })).toEqual({
      status: 'zero_amount',
    });
  });
